...
function useDocsSearchVersionsHelpers() {
  ...
  const [searchVersions, setSearchVersions] = useState(() =>
    Object.entries(allDocsData).reduce((acc, [pluginId, pluginData]) => {
      acc[pluginId] = pluginData.versions[0].name
      return acc
    }, {}),
  )
...
function SearchPageContent() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: makeSearch is static
  useEffect(() => {
    searchResultStateDispatcher({ type: 'reset' })
    if (searchQuery) {
        ...
        <form className="row" onSubmit={(e) => e.preventDefault()}>
          ...
          >
            <input
              ...
              value={searchQuery}
              autoComplete="off"
              // biome-ignore lint/a11y/noAutofocus: Required to focus the input on page load
              autoFocus
            />
          </div>
        ...
        {searchResultState.items.length > 0 ? (
          <main>
            {searchResultState.items.map(
              ({ title, url, summary, breadcrumbs }) => (
                <article key={`${title}`} className={styles.searchResultItem}>
                  ...
                  {breadcrumbs.length > 0 && (
                    <nav aria-label="breadcrumbs">
                      ...
                      >
                        {breadcrumbs.map((html) => (
                          <li
                            key={html}
                            className="breadcrumbs__item"
                            // biome-ignore lint/security/noDangerouslySetInnerHtml: Developer provided the HTML, so assume it's safe.
